...
static rlm_rcode_t mod_pre_proxy(void *instance, REQUEST *request)
{
	...
	rad_assert(request->packet != NULL);
	rad_assert(request->proxy-request != NULL);
	...
	if (inst->config->authorize_check_query) {
		...
		RDEBUG2("User found in radcheck table");
		user_found = true;
		if (paircompare(request, request->packet->vps, check_tmp, &request->proxy-request->vps) != 0) {
			pairfree(&check_tmp);
			check_tmp = NULL;
			goto skipreply;
		}
	...
	if (inst->config->authorize_reply_query) {
		...
		rows = sql_getvpdata(request->proxy-request, inst, request, &handle, &reply_tmp, expanded);
		TALLOC_FREE(expanded);
		if (rows < 0) {
			REDEBUG("SQL query error getting reply attributes");
			rcode = RLM_MODULE_FAIL;
		...
		radius_pairmove(request, &request->proxy-request->vps, reply_tmp, true);
